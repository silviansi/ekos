...
// Ambil data kos milik pemilik yang sedang login
$userId = $_SESSION['user_id'];
$stmt = $conn->prepare("SELECT * FROM kosts WHERE owner_id = ?");
$stmt->execute([$userId]);
                        ...
                        <div class="card table-card latest-activity-card">
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    ...
                                    <tbody>
                                        <?php $no = 1; foreach ($kosList as $kos): ?>
                                        <tr>
                                            <td><?= $no++ ?></td>
                                            <td><?= htmlspecialchars($kos['name']) ?></td>
                                            <td><?= htmlspecialchars($kos['address']) ?></td>
                                            <td>Rp<?= number_format($kos['price'], 0, ',', '.') ?></td>
                                            <td>
                                                <?php if ($kos['kost_status'] == 'active'): ?>
